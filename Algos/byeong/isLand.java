...
// 섬과바다 표시배열 int[][] island; 
// 섬과바다 검사여부배열 boolean[][] isCheck; 
...
Main() throws IOException { 
  ...
  if (inputLine.equals("00") || inputLine.equals("0 0")) 
      break; // line 1. 
      ...
      // 섬이 붙어있는 지 검사 시에 검사대상 주위를 둘러싼 인덱스를 구하기위해 IndexOutofBounds가 안 나게 하기위해 island = new int[height+2][width+2]; 
      // line x(2~height) 
      // island[][] 배열값 채워넣기 
      ...
      for (int n = 1; n < height+1; n++) { 
       ...
       while(token.hasMoreTokens()){ 
          ...
          island[n][tokenIndex] = element; tokenIndex++; } 
       } //end for 
    ...
    for(int i=0; i<search_h.length; i++){ 
        ...
        int around_w = w + search_w[i]; // 주위에 1인 위치를 발견하면 재귀호출하여 그 위치에서도 근처에 1이 있는지 탐색. 
        
        if(island[around_h][around_w]==1){ g
            etIslands(around_h, around_w); 
        } 
    } //end for 
